...
void input_init(ProgContext *ctx)
{
    ...
    glm_vec3_one(data->rendering.dirLight.dir);

    glm_vec3_one(data->rendering.dirLight.color);

    glm_vec3_zero(data->rendering.modelRotation);

    //Werte fuer die Punktlichtquelle initialisieren
    glm_vec3_zero(data->rendering.pointLight.middle);
    data->rendering.pointLight.middle[1] = 2.0f;
    glm_vec3_one(data->rendering.pointLight.color);
    glm_vec3_zero(data->rendering.pointLight.position);
    data->rendering.pointLight.position[1] = data->rendering.pointLight.middle[1];
    data->rendering.pointLight.radius = 1.0f;
    data->rendering.pointLight.active = true;
    data->rendering.pointLight.paused = false;

    //Allgemeine Werte fuers rendern initialisieren 
    data->rendering.scale = 1.0f;
    data->rendering.heightScale = 0.1f;
    ...
    data->rendering.useParallaxMapping = false;
    data->rendering.useTessellation = false;
    data->rendering.useDisplacement = false;
    data->rendering.userModel = NULL;
    data->rendering.sphereModel = NULL;

    //FBX-Datei fuer die Punktlichtquelle laden
    Model *newModel = NULL;
    newModel = model_loadModel("..\\res\\model\\Sphere.fbx");

    // Wir tauschen das Modell nur aus, wenn es erfolgreich geladen werden
    // konnte.
    if (newModel != NULL)
    {
        ctx->input->rendering.sphereModel = newModel;
    }
