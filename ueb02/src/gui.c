...
static void gui_widgetDisplayShader(struct nk_context *nk, const char *name, shaderDisplayOptions *opt)
{
    ...
    *opt = nk_option_label(nk, "Height-Map", *opt == HEIGHT_MAP) ? HEIGHT_MAP : *opt;
}
...
static void gui_renderMenu(ProgContext *ctx, struct nk_context *nk)
{
    ...
    if (input->showMenu)
    {
                                ...
                                NK_WINDOW_MINIMIZABLE | NK_WINDOW_TITLE))
        {
            ...
            {   
                //Light Direction
                gui_widgetVec3(nk, "Light Direction:", input->rendering.dirLight.dir);

                //Light Color
                gui_widgetLightColor(nk, "Light Color:", input->rendering.dirLight.color);

                //Point Light active
                nk_bool usePointLight = input->rendering.pointLight.active;
                if (nk_checkbox_label(nk, "Activate Point Light", &usePointLight))
                {
                    input->rendering.pointLight.active = usePointLight;
                }
                //Point Light Paused
                nk_bool pausePointLight = input->rendering.pointLight.paused;
                if (nk_checkbox_label(nk, "Pause Rotation", &pausePointLight))
                {
                    input->rendering.pointLight.paused = pausePointLight;
                }
                //Point Light Position
                gui_widgetVec3(nk, "Point Light Position:", input->rendering.pointLight.position);
                //Point Light Middle
                gui_widgetVec3(nk, "Middle:", input->rendering.pointLight.middle);
                //Point Light Color
                gui_widgetLightColor(nk, "Point Light Color:", input->rendering.pointLight.color);
                //Point Light Rotation Radius
                nk_labelf(nk, NK_TEXT_LEFT, "Radius:");
                nk_slider_float(nk, 0.5f, &input->rendering.pointLight.radius, 50, 0.1f);
            ...
            if (nk_tree_push(nk, NK_TREE_TAB, "Mapping", NK_MINIMIZED))
            {
                ...
                nk_bool parallaxMapping = input->rendering.useParallaxMapping;
                if (nk_checkbox_label(nk, "Parallaxmapping", &parallaxMapping))
                {
                    input->rendering.useParallaxMapping = parallaxMapping;
                }
                //Parallax Hoehe einstellen
                nk_labelf(nk, NK_TEXT_LEFT, "Height-Scale:");
                nk_slider_float(nk, 0.0f, &input->rendering.heightScale, 1, 0.1f);

                //Tessellation An/Aus schalten
                nk_bool useTessellation = input->rendering.useTessellation;
                if (nk_checkbox_label(nk, "Tessellation", &useTessellation))
                {
                    input->rendering.useTessellation = useTessellation;
                }
                //Innere Tesselation einstellen
                nk_labelf(nk, NK_TEXT_LEFT, "Inner Tessellation:");
                nk_slider_float(nk, 1.0f, &input->rendering.innerTessellation, 50, 1.0f);

                //Aussere Tesselation einstellen
                nk_labelf(nk, NK_TEXT_LEFT, "Outer Tessellation:");
                nk_slider_float(nk, 1.0f, &input->rendering.outerTessellation, 50, 1.0f);
