...
static void gui_widgetScale(struct nk_context *nk, const char *name, float *scale)
{
    ...
    nk_property_float(nk, "Height Scale", 0.01f, scale, 4.0f, *scale, 0.1f);
}
...
static void gui_renderMenu(ProgContext *ctx, struct nk_context *nk)
{
    ...
    if (input->showMenu)
    {
                                ...
                                NK_WINDOW_MINIMIZABLE | NK_WINDOW_TITLE))
        {
            ...
            {
                //DirLight aktivieren
                nk_bool dirLight = input->lighting.dirLightActive;
                if (nk_checkbox_label(nk, "DirLight (de-)aktivieren", &dirLight))
                {
                    input->lighting.dirLightActive = dirLight;
                }

                //Light Direction
                gui_widgetVec3(nk, "DirLight Direction:", input->lighting.dirLight.direction);
                //Light Color
                gui_widgetLightColor3(nk, "DirLight Color", input->lighting.dirLight.ambient, input->lighting.dirLight.diffuse, input->lighting.dirLight.specular);

                //DirLight aktivieren
                nk_bool pointLight = input->lighting.pointLightActive;
                if (nk_checkbox_label(nk, "Punktlichter (de-)aktivieren", &pointLight))
                {
                    input->lighting.pointLightActive = pointLight;
                }
                nk_tree_pop(nk);
            }
            ...
            if (nk_tree_push(nk, NK_TREE_TAB, "Mapping", NK_MINIMIZED))
            {
                ...
                nk_bool normalMapping = input->mapping.useNormalMapping;
                if (nk_checkbox_label(nk, "Normalmapping", &normalMapping))
                {
                    input->mapping.useNormalMapping = normalMapping;
                }

                //Parallaxmapping An/Aus schalten
                nk_bool parallaxMapping = input->mapping.useParallax;
                if (nk_checkbox_label(nk, "Parallaxmapping", &parallaxMapping))
                {
                    input->mapping.useParallax = parallaxMapping;
                }
                //Height Scale einstellen
                nk_property_float(nk, "Height Scale", 0.001f, &input->mapping.heightScale, 2.0f, 0.001f, 0.001f);

                //Tessellation An/Aus schalten
                nk_bool useTessellation = input->tessellation.useTessellation;
                if (nk_checkbox_label(nk, "Manual-Tessellation", &useTessellation))
                {
                    input->tessellation.useTessellation = useTessellation;
                    if (useTessellation)
                    {
                        input->tessellation.useDistanceTessellation = false;
                    }
                }
                //Innere Tesselation einstellen
                nk_property_float(nk, "Inner Tessellation:",  1.0f, &input->tessellation.innerTessellation, 50.0f, 1.0f, 1.0f);

                //Aussere Tesselation einstellen
                nk_property_float(nk, "Outer Tessellation:",  1.0f, &input->tessellation.outerTessellation, 50.0f, 1.0f, 1.0f);

                //Tessellation An/Aus schalten
                nk_bool useDistTessellation = input->tessellation.useDistanceTessellation;
                if (nk_checkbox_label(nk, "Distance-Tessellation", &useDistTessellation))
                {
                    input->tessellation.useDistanceTessellation = useDistTessellation;
                    if (useDistTessellation)
                    {
                        input->tessellation.useTessellation = false;
                    }
                }
                //Tessellation Amount einstellen
                nk_property_float(nk, "LoD:", 1.0f, &input->tessellation.tessellationAmount, 128.0f, 0.5f, 0.5f);

                //Displacement An/Aus schlaten
                nk_bool displacement = input->mapping.useDisplacement;
                if (nk_checkbox_label(nk, "Displacement", &displacement))
                {
                    input->mapping.useDisplacement = displacement;
                }
                //Displacement Faktor einstellen
                nk_property_float(nk, "Displacement Factor:", -0.1f, &input->mapping.displacementFactor, 0.1f, 0.001f, 0.001f);
                nk_tree_pop(nk);
            }
            ...
            {
                //DirLight aktivieren
                nk_bool useBloom = input->postProcessing.useBloom;
                if (nk_checkbox_label(nk, "Bloom (de-)aktivieren", &useBloom))
                {
                    input->postProcessing.useBloom = useBloom;
                }
                //Threshhold-Value einstellen
                nk_property_float(nk, "Threshhold value:", 0.0f, &input->postProcessing.threshhold, 15.0f, 0.01f, 0.01f);
                //Color-Weight einstellen
                nk_property_float(nk, "Bloom color weight:", 0.0f, &input->postProcessing.colorWeight, 15.0f, 0.01f, 0.01f);
                //Emission-Weight einstellen
                nk_property_float(nk, "Bloom emission weight:", 0.0f, &input->postProcessing.emissionWeight, 15.0f, 0.01f, 0.01f);
                //Blur-Iterationen einstellen
                nk_property_int(nk, "Blur Iterationen:", 0, &input->postProcessing.blurIterations, 10, 1, 1);
                //Gamma-Value einstellen
                nk_property_float(nk, "Gamma value:", 0.0f, &input->postProcessing.gamma, 10.0f, 0.01f, 0.01f);
                //Exposure-Value einstellen
                nk_property_float(nk, "Exposure value:", 0.0f, &input->postProcessing.exposure, 10.0f, 0.01f, 0.01f);

                nk_tree_pop(nk);
            }
            // Einstellungen bezüglich des ShadowMappings.
            if (nk_tree_push(nk, NK_TREE_TAB, "Shadow-Mapping", NK_MINIMIZED))
            {
                //Dir Shadows rendern
                if (nk_button_label(nk, "Reload Directional Shadows"))
                {
                    input->shadows.createDirShadows = !input->shadows.createDirShadows;
                }
                //Point Shadows rendern
                nk_bool calcPointShadows = input->shadows.createPointLightShadows;
                if (nk_checkbox_label(nk, "Load PointLight Shadows", &calcPointShadows))
                {
                    input->shadows.createPointLightShadows = calcPointShadows;
                    input->shadows.showPointShadows = calcPointShadows;
                }
                nk_bool showDirShadows = input->shadows.showDirShadows;
                if (nk_checkbox_label(nk, "Show Directional Shadows", &showDirShadows))
                {
                    input->shadows.showDirShadows = showDirShadows;
                }
                nk_bool showPointShadows = input->shadows.showPointShadows;
                if (nk_checkbox_label(nk, "Show Point Shadows", &showPointShadows))
                {
                    input->shadows.showPointShadows = showPointShadows;
                }
                //Echtzeit Dir Shadows aktivieren
                nk_bool realtimeDirShadows = input->shadows.realtimeDirShadows;
                if (nk_checkbox_label(nk, "Realtime Directional Shadows", &realtimeDirShadows))
                {
                    input->shadows.realtimeDirShadows = realtimeDirShadows;
                }
                //PCF aktivieren
                nk_bool usePCF = input->shadows.usePCF;
                if (nk_checkbox_label(nk, "Use PCF", &usePCF))
                {
                    input->shadows.usePCF = usePCF;
                }

                nk_property_int(nk, "PCF Amount:", 1, &input->shadows.PCFAmount, 10, 1, 1);

                //PCF aktivieren
                nk_bool useBilinearFiltering = input->shadows.useBilinearFiltering;
                if (nk_checkbox_label(nk, "Use Bilinear Filtering", &useBilinearFiltering))
                {
                    input->shadows.useBilinearFiltering = useBilinearFiltering;
                }

                nk_tree_pop(nk);
            }

            // Einstellungen bezüglich der Partikel.
            if (nk_tree_push(nk, NK_TREE_TAB, "Particles", NK_MINIMIZED))
            {
                // Animation Pausieren einstellen
                nk_bool pausePart = input->particles.pauseSim;
                if (nk_checkbox_label(nk, "Pause Particles", &pausePart))
                {
                    input->particles.pauseSim = pausePart;
                }

                //Lebenszeit-Value einstellen
                nk_property_float(nk, "Lebenszeit:", 2.0f, &input->particles.lifeTime, 15.0f, 0.1f, 0.1f);
                //LZZufall-Value einstellen
                nk_property_float(nk, "L.Z. Zufall:", 0.0f, &input->particles.lifeTimeRand, 15.0f, 0.01f, 0.01f);

                //Startpunkt Direction
                gui_widgetVec3(nk, "Startpunkt:", input->particles.startPos);

                //Startrichtung Direction
                gui_widgetVec3(nk, "Startrichtung:", input->particles.startDir);
                //S.R Zufall-Value einstellen
                nk_property_float(nk, "S.R. Zufall:", 0.0f, &input->particles.startDirRand, 15.0f, 0.1f, 0.1f);

                //Gravity Direction
                gui_widgetVec3(nk, "Gravitation:", input->particles.gravity);

                // Start Color
                gui_widgetColor(nk, "Startfarbe:", input->particles.startColor);

                // End Color
                gui_widgetColor(nk, "Endfarbe:", input->particles.endColor);

                //StartSize-Value einstellen
                nk_property_float(nk, "Startgrosse:", 0.0f, &input->particles.startSize, 1.0f, 0.1f, 0.1f);
                //EndSize-Value einstellen
                nk_property_float(nk, "Endgrosse:", 0.0f, &input->particles.endSize, 1.0f, 0.1f, 0.1f);
