...
Shader* shader_createCompShader(const char* comp)
{
    ...
    bool compOk = shader_attachShaderFile(newShader, GL_COMPUTE_SHADER, comp);
    ...
    if (compOk)
    {
        ...
        {
            // Wenn dies erfolgreich war, geben wir den neuen Shader zurück.
            return newShader;
        }
    }
...
Shader *shader_createVeFrShader(const char *vert, const char *frag)
{
    // Zuerst werden alle benötigten Bestandteile des Shaders angelegt,
    // egal ob einer Fehler verursacht.
    Shader *newShader = shader_createShader();
...
Shader *shader_createVeGeomFrShader(const char *vert, const char *geom, const char *frag)
{
    // Zuerst werden alle benötigten Bestandteile des Shaders angelegt,
    // egal ob einer Fehler verursacht.
    Shader *newShader = shader_createShader();
...
Shader *shader_createVeCoEvFrShader(const char *vert, const char *frag, const char *control, const char *eval)
{
    // Zuerst werden alle benötigten Bestandteile des Shaders angelegt,
    // egal ob einer Fehler verursacht.
    Shader *newShader = shader_createShader();
    if (eval != NULL && control != NULL)
    {
        bool vertOk = shader_attachShaderFile(newShader, GL_VERTEX_SHADER, vert);
        bool contOk = shader_attachShaderFile(newShader, GL_TESS_CONTROL_SHADER, control);
        bool evalOk = shader_attachShaderFile(newShader, GL_TESS_EVALUATION_SHADER, eval);
        bool fragOk = shader_attachShaderFile(newShader, GL_FRAGMENT_SHADER, frag);
        // Danach wird auf mögliche Fehler geprüft.
        if (vertOk && fragOk && evalOk && contOk)
        {
            // Wenn keine Fehler aufgetreten sind, kann der Shader gebaut werden.
            if (shader_buildShader(newShader))
            {
