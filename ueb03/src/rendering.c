    ...
    if (input->rendering.userScene)
    {
        ...
        if (input->rendering.userScene->model)
        {
            ...
            if ((data->modelShader) && (data->null) && (data->pointLight) && (data->dirLight))
            {
                ...
                if (input->rendering.pointLightActive)
                {
                ...
                if (input->rendering.dirLightActive)
                {
                    deferredShader_activateTexturesLighting(data);
                    deferredShader_doDirLightPass(ctx);
                }
            }
            ...
            if (data->blur)
            {
                horizontal = postProcessing_blur(ctx);
            }

            /* ---------------------- Post-Process - SHADER ---------------------------------- */
            if (data->postProcessing)
            {
                deferredShader_activateTexturesFinalRender(data, horizontal);
